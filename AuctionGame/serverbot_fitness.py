...
def parseNums(playerid):
	...
	for char in playerid:
		if char == "_":
			...
			playerNums[flag] = playerNums[flag] + char
	return playerNums
# parseNums(..)
...
while(0 == doneflag):
  ...
  if (won[mytype][winnerid] >= neededtowin):
    ...
    if platform.system() == 'Windows':
      ...
      os.system('clear')
    for playerid in bidderids:
        if "genetic" in playerid:
            playerNums = parseNums(playerid)
            fileName = "fitness\\gen_" + playerNums[0] + "\\" + playerNums[1]
            fitFile = open(fileName, "a")
            fitness = 0
            for artist in itemtypes:
                if won[artist][playerid] == 1:
			        fitness = fitness + 1
                if won[artist][playerid] == 2:
                    fitness = fitness + 5
